...
const AllBook = ({ location, locationCoordinates }) => {
  ...
  const { bookData, loading } = getAllBookings();

  const filteredBookData = bookData.filter(item => !item.isDropoff && (!item.driverId || item.driverId === auth.currentUser.uid));
        ...
        <View style={styles.bookingContainer}>
          {filteredBookData.length > 0 ? (
            ...
            <Text style={styles.noBookingText}>
              {bookData.length === 0 ? "They haven't booked a ride yet." : "No booking information found."}
            </Text>
          )}
        </View>
      )}
